...
public class MB2_MeshBaker : MB2_MeshBakerCommon {	
	...
	public void BuildSceneMeshObject(){
		...
		_update_MB2_MeshCombiner();
		MB_Utility.buildSceneMeshObject(resultSceneObject, meshCombiner, meshCombiner.GetMesh());
	}
	...
	public override Mesh AddDeleteGameObjects(GameObject[] gos, GameObject[] deleteGOs, bool disableRendererInSource, bool fixOutOfBoundUVs){
		if (resultSceneObject == null && (meshCombiner.outputOption == MB2_OutputOptions.bakeIntoSceneObject || (meshCombiner.outputOption == MB2_OutputOptions.bakeIntoPrefab && meshCombiner.renderType == MB_RenderType.skinnedMeshRenderer))) BuildSceneMeshObject();
		_update_MB2_MeshCombiner();
	...
	public override void RebuildPrefab(){
		if (renderType == MB_RenderType.skinnedMeshRenderer){
			Debug.LogWarning("Render type is skinned mesh renderer. All bones (source game objects) need to be included in the prefab. 'Combined Mesh Prefab' " + resultPrefab + " should be instantiated in the scene. Add the objects to be combined to this prefab an click apply.");	
		}
