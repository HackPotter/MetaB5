...
public class ImageHeader : MonoBehaviour {
        ...
        try {
            using (BinaryReader binaryReader = new BinaryReader(File.OpenRead(path))) {
                try {
                    ...
                    // throw new ArgumentException(newMessage, "InnerException", e.InnerException);
                    Debug.LogError("ImageHeader.cs\\GetDimensions InnerException: " + e.InnerException);
                    Debug.LogError("ImageHeader.cs\\GetDimensions StackTrace: " + e.StackTrace);
                    return new Vector2Int(0, 0);
                }
            }
        }
        catch (Exception ex) {
            Debug.LogError("ImageHeader.cs\\GetDimensions ERROR: " + ex.Message);
            return new Vector2Int(0, 0);
        }
    }
    ...
    public static Vector2Int GetDimensions(BinaryReader binaryReader) {
        int maxMagicBytesLength = -1;
        try {
            maxMagicBytesLength = imageFormatDecoders.Keys.OrderByDescending(x => x.Length).First().Length;
            //Debug.Log("int maxMagicBytesLength: " + maxMagicBytesLength);
            ...
            Debug.LogError("ImageHeader.cs\\GetDimensions InnerException: " + ex.InnerException);
            Debug.LogError("ImageHeader.cs\\GetDimensions StackTrace: " + ex.StackTrace);
