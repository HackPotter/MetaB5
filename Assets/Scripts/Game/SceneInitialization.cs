...
#pragma warning restore 0067, 0649
    ...
    void OnLevelWasLoaded(int level)
    {
        ...
        {
            AnalyticsLogger.Instance.AddLogEntry(new SceneLoadedLogEntry(GameContext.Instance.Player.UserGuid, Application.loadedLevelName, level));
            return;
        }
        RememberMe.Clear();
        ...
        if (!_sceneInitialized)
        {
            ...
            AnalyticsLogger.Instance.AddLogEntry(new SceneLoadedLogEntry(GameContext.Instance.Player.UserGuid, Application.loadedLevelName, level, 0f));
        }
        else
        {
            AnalyticsLogger.Instance.AddLogEntry(new SceneLoadedLogEntry(GameContext.Instance.Player.UserGuid, Application.loadedLevelName, level));
